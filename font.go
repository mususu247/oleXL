...
type workFort struct {
	app *Excel
	mx  int
}

func (rg *workRag) Font() *workFort {
	var ig workFort
	ig.app = rg.app
	xl := rg.app
	...
	for i := range args {
		...
		case *ole.IDispatch:
			...
			ig.mx, _ = xl.addCore(rg.mx, x, "Font", 0)
			return &ig
		default:
...
func (ft *workFort) ColorIndex(value ...any) int32 {
	var result int32
	xl := ft.app
	_core, err := xl.getCore(ft.mx)
	if err != nil {
		return -1
	}
	var cmd string
...
func (ft *workFort) Color(value ...float64) float64 {
	xl := ft.app
	_core, err := xl.getCore(ft.mx)
	if err != nil {
